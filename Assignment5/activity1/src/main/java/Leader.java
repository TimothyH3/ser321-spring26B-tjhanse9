...
public class Leader {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            // accept worker connections in a separate thread
                ...
                try {
                    while (true) {
                        ...
                        synchronized (workers) {
                            workers.add(worker);
                            worker.start();
                            Leader.workerCount.incrementAndGet();
                            System.out.println("Connected workers: " + Leader.workerCount.get());
                        }
                    }
                } catch (IOException e) {
                    System.out.println("Connection thread stopped: " + e.getMessage());
                }
            });
            connectionThread.start();
            
            // wait for minimum workers before starting consensus
            while (Leader.workerCount.get() < expectedWorkers) {
                Thread.sleep(100);
            }
            
            System.out.println("Minimum workers reached. Starting consensus rounds...");
            ...
            while (true) {
                ...
                // collect responses from current workers
                int currentWorkers;
                synchronized (workers) {
                    currentWorkers = workers.size();
                }
                ...
                double percentage = (double) maxVotes / currentWorkers;
                String finalAnnouncement;
                
                if (percentage >= 0.5 && maxVotes > 0) {
                    finalAnnouncement = "Consensus: " + consensusAnswer + " (" + maxVotes + "/" + currentWorkers + " workers agreed)";
