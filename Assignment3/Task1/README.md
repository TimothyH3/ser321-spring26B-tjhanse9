...
This service performs a mathematical operation (add, multiply, or average) on an array of integers provided by the client.

Request:

{
    "type" : "calculatemany",
    "operation" : <String>, -- The operation to perform: "add", "multiply", or "average" (case-insensitive)
    "numList" : [<int>] -- A JSON Array of integers (e.g., [1, 2, 3])
}
General response:

{
    "type" : "calculatemany",
    "ok" : <bool>, -- true or false depending on request
    "operation" : <String>, -- echoes the requested operation if ok is true
    "count" : <int>, -- the number of integers processed if ok is true
    "sum" : <int>, -- the result if operation was 'add'
    "product" : <int>, -- the result if operation was 'multiply'
    "average" : <double>, -- the result if operation was 'average' (rounded to 2 decimals)
    "message" : <String> -- error message if ok false
}
Success response (Operation: add):
...
Success response (Operation: multiply):
...
Success response (Operation: average):
...
{
    "ok" : false,
    "message" : "Invalid operation '<invalid_op>'. Valid operations: add, multiply, average"
}
Error response (Non-integer values in array):
